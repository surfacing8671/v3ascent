...
function updateCreateAccount(
    Account: accountsEntity | undefined,
    AccountAddress: string,
    Token: tokenEntity,
    Amount: BigInt,
): accountsEntity {
    if (!!Account) {
        const updateAccountBalance: accountsEntity = {
          id: Account.id,
          tokenBalance: Account.tokenBalance + Amount,
          numberOfTokens: 0
        };
        return updatedLiquidatorAccount;
    } else {
        const liquidatorAccountObject: accountsEntity = {
